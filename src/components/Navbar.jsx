...
function Navbar() {
  ...
  return (
    <nav className="fixed top-0 w-full z-40 bg-background/80 backdrop-blur-md border-b border-border">
      ...
      {isOpen && (
        <div className="md:hidden">
          <div className="px-6 pt-2 pb-3 space-y-1 bg-background/95 backdrop-blur-md">
            {navItems.map((item) => (
              <a
                key={item.href}
                href={item.href}
                className="text-foreground hover:text-primary block px-3 py-2 rounded-md text-base font-medium hover:bg-accent transition-colors duration-300 text-center"
